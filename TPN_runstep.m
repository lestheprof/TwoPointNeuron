...
while ((apical(tpnno).apicalspikeno <= size(apical(tpnno).apicalinputs,1)) && ...
    ...
    apical(tpnno).alphalen = alphalen ; % usable for resetting after spiking
end
...
if (ts > 1)
    % Jan 24 2025: decrement is towards reset value, not 0
    decrement = (apicalactivation(tpnno,ts-1) - apical(tpnno).resetvalue) * apical(tpnno).apicalfracleak ;
    apicalactivation(tpnno,ts) = apicalactivation(tpnno,ts-1) - decrement ... % decrement due to leakage
...
if (ts > 1)
    % Jan 24 2025: decrement is towards reset value, not 0
    decrement = (basalactivation(tpnno,ts-1) - basal(tpnno).resetvalue) * basal(tpnno).basalfracleak ;
    basalactivation(tpnno,ts) = basalactivation(tpnno,ts-1) - decrement + ...
...
% might do better to consider deflection from reset/initial value rather
% than actual values.
